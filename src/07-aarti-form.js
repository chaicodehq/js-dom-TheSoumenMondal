...
export function setupAartiForm(formElement, onSuccess, onError) {
  ...
  const handler = (event) => {
    ...
    const validations = [validateName(name), validateDate(date), validateAartiType(aartiType)];
    const errors = validations.filter((item) => !item.valid).map((item) => item.error);
...
export function createBookingSummary(booking) {
  ...
  wrapper.classList.add('booking-summary');

  const title = document.createElement('h3');
  title.textContent = 'Booking Confirmation';

  const name = document.createElement('p');
  name.classList.add('booking-name');
